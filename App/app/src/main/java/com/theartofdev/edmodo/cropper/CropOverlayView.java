...
public class CropOverlayView extends View {
  ...
  /** The Paint used to draw the optional white rectangle around the crop area. */
  private Paint mOptBorderPaint;
  ...
  /** the length of the optional border corner to draw */
  private float mOptBorderCornerLength;
   ...
   */
  private float mTargetAspectRatio = 1.0f; // ((float) mAspectRatioX) / mAspectRatioY;

  /** The type of cropping screen - common/optional */
  private CropImageView.CropScreenType mCropScreenType;

  /** Instance variables for customizable attributes */
  private CropImageView.Guidelines mGuidelines;

  private CropImageView.Conners mConners;
  ...
  /** The type of cropping screen - common/optional. */
  public CropImageView.CropScreenType getCropScreenType(){
    return mCropScreenType;
  }

  /** The type of cropping screen - common/optional. */
  public void setCropScreenType(CropImageView.CropScreenType cropScreenType){
    if(mCropScreenType != cropScreenType){
      mCropScreenType = cropScreenType;
      invalidate();
    }
  }
  ...
  public void setConners(CropImageView.Conners conners) {
    if (mConners != conners) {
      mConners = conners;
      if (initializedCropWindow) {
        invalidate();
  ...
  public void setInitialAttributeValues(CropImageOptions options) {
    ...
    setCropScreenType(options.cropScreenType);
    ...
    mOptBorderPaint = getNewPaintOrNull(options.optBorderLineThickness, options.optBorderLineColor);
    mBorderCornerOffset = options.borderCornerOffset;
    mBorderCornerLength = options.borderCornerLength;
    mOptBorderCornerLength = options.optBorderCornerLength;
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mConners == CropImageView.Conners.ON) {
      drawCorners(canvas);
    }
  ...
  private void drawBorders(Canvas canvas) {
    if(mCropScreenType == CropImageView.CropScreenType.COMMON) {
      // Draw border for profile crop screen
      ...
      // Draw border for photo drawing crop screen
      if(mOptBorderPaint != null){
        float w = mOptBorderPaint.getStrokeWidth();
        RectF rect = mCropWindowHandler.getRect();
        rect.inset(w / 2, w / 2);

        canvas.drawRect(rect, mOptBorderPaint);
      }
    }
  }
  ...
  private void drawCorners(Canvas canvas) {
    if (mBorderCornerPaint != null) {
      ...
      float cornerLength = (mCropScreenType == CropImageView.CropScreenType.COMMON) ?
                            mBorderCornerLength : mOptBorderCornerLength;

      // For option screen type, we allow the corner to be offset from the borders
      float w = (mCropScreenType == CropImageView.CropScreenType.OPTIONAL) ?
              (mBorderCornerOffset + cornerWidth) : 0;
      RectF rect = mCropWindowHandler.getRect();
      rect.inset(w, w);

      float cornerOffset = cornerWidth / 2 - lineWidth;
